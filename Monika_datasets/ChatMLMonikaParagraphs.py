...
class DialogueExtractor:
    ...
    def format_to_chatml(self, dialogue_entries):
        """Convert grouped dialogue entries to ChatML format."""
        output = []
        ...
        for i, entry in enumerate(grouped_entries):
            if entry['type'] == 'assistant':
                ...
                if entry.get('emotions') or (i > 0 and grouped_entries[i-1].get('type') == 'user'):
                    output.append("<|im_start|>system")
                    ...
                    if i > 0 and grouped_entries[i-1].get('type') == 'user':
                ...
                output.append("<|im_end|>")
                prev_assistant_msg = entry['text'].split('\n')[-1]  # Store last line for context
