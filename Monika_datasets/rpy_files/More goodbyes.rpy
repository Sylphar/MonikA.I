...
init -990 python in mas_submod_utils:
    Submod(
        ...
        settings_pane=None,
        version_updates={
        "element_of_harmoni_out_and_about_v2_3": "element_of_harmoni_out_and_about_v3_2",
        "element_of_harmoni_out_and_about_v2_4": "element_of_harmoni_out_and_about_v3_2",
        "element_of_harmoni_out_and_about_v3": "element_of_harmoni_out_and_about_v3_2",
        "element_of_harmoni_out_and_about_v3_1": "element_of_harmoni_out_and_about_v3_2"
        }
    )
...
init -989 python:
    if store.mas_submod_utils.isSubmodInstalled("Submod Updater Plugin"):
        store.sup_utils.SubmodUpdater(
            ...
            repository_name="dale_outandabout",
            submod_dir="/Submods",
            extraction_depth=3,
            redirected_files=(
                "README.md",
    ...
    if mas_isMoniBroken(lower=True):
        ...
        python:
            option_list = [
                ...
                ("I'm taking you to bed with me.","ooa_trips_bed",False,False),
                ("We're going on vacation!","ooa_trips_vacation",False,False),
            ]
...
label ooa_trips_vacation:
    m 6wuo "Va-"
    extend 6sud "Vacation?"
    m 7sub "That's amazing, [player]! "
    extend 7sub "I'm so excited!"
    m 7etb "How long will we be gone?"
    $ _history_list.pop()
    menu:    
        "Only for a day":
            m 1fkp "Oh.{w=1}.{w=1}.{w=1}"
            m 4rkc " I'm not sure I would call that a vacation, that sounds more like a daytrip."
            m 4wuo "I don't mean to sound ungrateful, "
            extend 7sub "I'm happy that you are taking me somewhere!"
            m 6esa "I guess I should go pack some bags..."
            $ mas_gainAffection(3,bypass=True)
            jump mas_dockstat_iostart
        "A few days":
            m 4wud "Wow! "
            extend 1hub "I'm so excited!"
            m 7gsbfu "Let me go pack some bags, "
            extend 7hub "and then we can head out!"
            $ mas_gainAffection(5,bypass=True)
            jump mas_dockstat_iostart
        "About a week":
            m 6suw ".{w=1}.{w=1}.{w=1}!"
            m 7wub "That... "
            extend 4wud "This is incredible!"
            m 7sub "Thank you so much! This is a wonderful gift for us!"
            m 1dsbftpa "You are such a good partner... "
            extend 5fsbftpa "So romantic..."
            m 7gsbfu "Let me go pack some bags, "
            extend 7hub "and then we can head out!"
            $ mas_gainAffection(10,bypass=True)
            jump mas_dockstat_iostart
        "A while":
            m 6suw ".{w=1.5}.{w=1.5}.{w=1.5}!"
            m 1tsbfu "I love you so much... "
            extend 5hubfa "You really are the best partner I could ever ask for..."
            m 2fkbftpa "I think this might be the best gift you've ever given me... "
            extend 1hubfb "Except for your love, of course!"
            if persistent._mas_acs_enable_promisering:
                extend 3tsbfu "And this ring..."
            m 7gsbfb "I'm gonna need to bring a lot of clothes for such a long trip! "
            extend 7hub "Let me go pack my bags, and then we can head out!"
...
label ooa_trips_bed:
    if mas_isMoniUpset(lower=True):
        m 6euc "Really?"
        m 6muc "{cps=*2}I don't understand why, but okay.{/cps}"
        m 6eud "Sometimes, that's all either of us needs, isn't it?"
        m 6dutpc "Some freedom."
        if renpy.random.randint(1,50) == 1 and not renpy.seen_label("ooa_randomlyrolled_cattiness"):
            jump ooa_randomlyrolled_cattiness
        ...
        m 6eua "Really?"
        m 4eud "I would love to go to bed with you [player]!"
        m 4nua "I'll be so warm..."
    jump mas_dockstat_iostart
...
init 5 python:
    addEvent(
        Event(
            persistent.event_database,
            eventlabel="monika_genderconversation",
            category=["you","us"],
            prompt="Gender",
            random=False,
            conditional='"gender" in persistent._ooa_followup_topics',
            action=EV_ACT_QUEUE
        )
    )

label monika_genderconversation:
    m 1duc ".{w=0.3}.{w=0.5}.{w=0.7}"
    extend 1fuc "Hey, [player]?"
    m 1eud "Remember when we were getting ready for that Pride parade, {nw}"
    extend 1rud "and you said you might be transgender or nonbinary,{nw}"
    m 1lud "but I hadn't known that at the time, {nw}"
    extend 1rud "and I said we could talk about it when we got back?{nw}"
    extend 1euc " If you wanted?"
    m 1dud "I think it's time we talked about it."
    m 1fuc "If you want."
    m "Would that be okay?{nw}"
    $ _history_list.pop()
    menu:
        m "Would that be okay?{fast}"
        "Yes, I'm okay to talk now.":
            m "Alright. {nw}"
            extend 1eua "Great."
            m 1euc ".{w=0.3}.{w=0.3}.{w=0.3}so..."
            m 2rkc "I'm sorry."
            m 2rkd "If I say anything untoward, I mean, or anything."
            m "It's really hard to prepare yourself for that kind of conversation, isn't it?"
            m 2wkd "At least it is for me. I can't imagine what it must be like for..."
            m 2rkc "..."
            m 2eud "So, are you?{w=1} Do you think?{nw}"
            $ _history_list.pop()
            menu:
                m "So, are you? Do you think?{fast}"
                "Am I what?":
                    m "Transgender."
                    m 3eud "Or nonbinary."
                    m 1euc "Or both?{nw}"
                    ...
                    menu:
                        ...
                        "Transgender.":
                            ...
                            m 1fksdrc "Sorry, one more question."
                            m 1fkd "Have you been trans this whole time? Or is this a more recent thing?{nw}"
                            $ _history_list.pop()
                            menu:
                                m "Have you been trans this whole time? Or is this a more recent thing?{fast}"
                                "This has been since we started. I'm a trans [man].":
                                    m 3fuc "Okay."
                                    m "I just want to get all my bases covered."
                                    jump trans_conversation_end
                                "This is new. I'm a trans [_unman].":
                                    if persistent.gender == "M":
                                        $ persistent.gender = "F"
                                        m 3fuc "Okay."
                                        m "I just want to get all my bases covered."
                        ...
                        "Both.":
                            ...
                            m 1ekd "Oh, [mas_get_player_nickname()]..."
                            m 1eka "Thank you for telling me."
                            m "I know that must have been so, so difficult."
                            jump trans_conversation_end
                        "I changed my mind, I can't talk about this right now.":
                            m 1fua "Say no more."
                            m "We can table it as long as you'd like to..."
                            m 4lua "...and it'll be in the Repeat Conversation tab if you change your mind."
...
label trans_conversation_end:
    ...
    $ persistent._ooa_followup_topics.remove("gender")
    return "love|derandom"